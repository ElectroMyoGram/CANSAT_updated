...
yaw_data = deque(maxlen=buffer_size)  # Added Yaw tracking
...
line_yaw, = ax.plot([], [], label="Yaw (°)", color="b")  # Added Yaw plot
    ...
    while True:
        ...
        if line:
            ...
            try:
                ...
                if len(values) == 3:  # Now expects 3 values (roll, pitch, yaw)
        ...
        if time.time() - last_update_time >= 0.01:
            ...
            line_yaw.set_ydata(list(yaw_data))  # Update Yaw data
